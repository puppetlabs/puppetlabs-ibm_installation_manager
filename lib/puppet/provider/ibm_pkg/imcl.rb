...
Puppet::Type.type(:ibm_pkg).provide(:imcl) do
  ...
  def create
    if resource[:response]
      ...
      cmd_options =  cmd_options + " #{resource[:jdk_package_name]}_#{resource[:jdk_package_version]}" unless resource[:jdk_package_name].nil?
      cmd_options =  cmd_options + " -repositories #{resource[:repository]} -installationDirectory #{resource[:target]}"
    else
      cmd_options =  "install #{resource[:package]}_#{resource[:version]}"
      cmd_options =  cmd_options + " #{resource[:jdk_package_name]}_#{resource[:jdk_package_version]}" unless resource[:jdk_package_name].nil?
      cmd_options =  cmd_options + " -repositories #{resource[:repository]} -installationDirectory #{resource[:target]} -accessRights nonAdmin"
    end
    cmd_options =  cmd_options + ' -acceptLicense'
    cmd_options =  cmd_options + " #{resource[:options]}" if resource[:options]
